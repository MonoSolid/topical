...
public class AsyncTopic<T> : IAsyncTopic<T>
    where T : notnull
{
    private ImmutableArray<AsyncTopicSubscription<T>> subscriptions = [];
    ...
    )
    {
        ...
        var filteredSubscription = new AsyncTopicSubscription<T>(where, asyncHandler, cts.Token);
